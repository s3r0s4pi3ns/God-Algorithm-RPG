...
describe("Player basic functionality", () => {
    it('should be allowed to switch programming language', () => {
        const player = new PlayerFactory().create({ programmingLanguage: PROGRAMMING_LANGUAGE.PHP })
    ...
    it('The Player swithProgrammingLanguage method should be chained and return the player instance', () => {
        ...
        expect(player.switchProgrammingLanguage(PROGRAMMING_LANGUAGE.CPLUS)).toBeInstanceOf(Player)
        expect(player.switchProgrammingLanguage(PROGRAMMING_LANGUAGE.CPLUS).id).toBe(player.id)

        player.switchProgrammingLanguage(PROGRAMMING_LANGUAGE.PHP)
    ...
    it("should be able to use the laptop if has battery", () => {
        const player = new PlayerFactory().create({ laptop: new LaptopFactory().create({ battery: 56 }) })
